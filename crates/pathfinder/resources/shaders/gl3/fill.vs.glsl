...
layout(location = 4) in uvec2 aTileIndex;
layout(location = 3) in uvec2 aPx;
...
void main()
{
    uint param = aTileIndex.x;
    float param_1 = uFramebufferSize[0].x;
    vec2 tileOrigin = computeTileOffset(param, param_1);
    vec2 from = vec2(float(aPx.x & 15u), float(aPx.x >> 4u)) + aFromSubpx;
    vec2 to = vec2(float(aPx.y & 15u), float(aPx.y >> 4u)) + aToSubpx;
    vec2 position;
    if (aTessCoord.x == 0u)
    {
