...
impl<'a, T: Component> Fetch<'a> for FetchRead<T> {
    ...
    #[inline]
    unsafe fn next(&mut self, _meta: &[EntityMeta]) -> &'a T {
        let x = self.0.as_ptr();
...
impl<'a, T: Component> Fetch<'a> for FetchWrite<T> {
    ...
    #[inline]
    unsafe fn next(&mut self, _meta: &[EntityMeta]) -> &'a mut T {
        let x = self.0.as_ptr();
        self.0 = NonNull::new_unchecked(x.add(1));
        &mut *x
    }
...
impl<'a, T: Fetch<'a>> Fetch<'a> for TryFetch<T> {
    ...
    unsafe fn next(&mut self, meta: &[EntityMeta]) -> Option<T::Item> {
        Some(self.0.as_mut()?.next(meta))
    }
}
...
impl<'a, T: Component, F: Fetch<'a>> Fetch<'a> for FetchWithout<T, F> {
    ...
    unsafe fn next(&mut self, meta: &[EntityMeta]) -> F::Item {
        self.0.next(meta)
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            match self.iter {
                ...
                Some(ref mut iter) => match unsafe { iter.next(self.borrow.meta) } {
                    None => {
                        self.iter = None;
                        continue;
                    }
...
impl<'q, 'w, Q: Query> Iterator for Batch<'q, 'w, Q> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let components = unsafe { self.state.next(self.meta)? };
        Some(components)
    }
}
