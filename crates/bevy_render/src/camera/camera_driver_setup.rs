use crate::{
    camera::{ClearColor, ExtractedCamera, NormalizedRenderTarget, SortedCameras},
    frame_graph::*,
    view::ExtractedWindows,
};
use bevy_ecs::{
    entity::{ContainsEntity, Entity},
    prelude::QueryState,
    world::World,
};
use bevy_platform::collections::HashSet;
use wgpu::{LoadOp, Operations, StoreOp};

pub struct CameraDriverSetup {
    cameras: QueryState<&'static ExtractedCamera>,
}

impl CameraDriverSetup {
    pub fn new(world: &mut World) -> Self {
        Self {
            cameras: world.query(),
        }
    }

    pub fn get_swap_chain_texture_key(entity: Entity) -> String {
        format!("swap_chain_texture_{}", entity)
    }
}

impl Setup for CameraDriverSetup {
    fn update(&mut self, world: &mut World) {
        self.cameras.update_archetypes(world);
    }
    fn run(
        &self,
        graph: &mut SetupGraphContext,
        frame_graph: &mut FrameGraph,
        world: &World,
    ) -> Result<(), SetupRunError> {
        let sorted_cameras = world.resource::<SortedCameras>();
        let windows = world.resource::<ExtractedWindows>();
        let mut camera_windows = <HashSet<_>>::default();
        for sorted_camera in &sorted_cameras.0 {
            let Ok(camera) = self.cameras.get_manual(world, sorted_camera.entity) else {
                continue;
            };

            let mut run_graph = true;
            if let Some(NormalizedRenderTarget::Window(window_ref)) = camera.target {
                let window_entity = window_ref.entity();
                if windows
                    .windows
                    .get(&window_entity)
                    .is_some_and(|w| w.physical_width > 0 && w.physical_height > 0)
                {
                    camera_windows.insert(window_entity);
                } else {
                    // The window doesn't exist anymore or zero-sized so we don't need to run the graph
                    run_graph = false;
                }
            }
            if run_graph {
                graph.run_sub_graph(camera.render_graph, vec![], Some(sorted_camera.entity))?;
            }
        }

        let clear_color_global = world.resource::<ClearColor>();

        // wgpu (and some backends) require doing work for swap chains if you call `get_current_texture()` and `present()`
        // This ensures that Bevy doesn't crash, even when there are no cameras (and therefore no work submitted).
        for (id, window) in world.resource::<ExtractedWindows>().iter() {
            if camera_windows.contains(id) {
                continue;
            }

            let Some(swap_chain_texture) = &window.swap_chain_texture else {
                continue;
            };

            let swap_chain_texture = FrameGraphTexture::new_arc(swap_chain_texture.texture.clone());

            let mut builder = frame_graph.create_pass_node_builder("camera_pass");

            let swap_chain_texture_handle = builder.import(
                &CameraDriverSetup::get_swap_chain_texture_key(*id),
                swap_chain_texture,
            );

            let swap_chain_texture_ref = builder.read(swap_chain_texture_handle);

            builder.set_pass_data(CameraPassData {
                render_pass_info: RenderPassInfo {
                    color_attachments: vec![ColorAttachmentInfo {
                        view: swap_chain_texture_ref,
                        resolve_target: None,
                        ops: Operations {
                            load: LoadOp::Clear(clear_color_global.to_linear().into()),
                            store: StoreOp::Store,
                        },
                    }],
                },
            });
        }

        Ok(())
    }
}

pub struct CameraPassData {
    render_pass_info: RenderPassInfo,
}

impl PassData for CameraPassData {
    fn execute(&self, render_context: &mut RenderContext) -> Result<()> {
        #[cfg(feature = "trace")]
        let _span = tracing::info_span!("no_camera_clear_pass").entered();
        let render_pass = render_context.begin_render_pass(&self.render_pass_info)?;
        render_pass.end_render_pass();

        Ok(())
    }
}
