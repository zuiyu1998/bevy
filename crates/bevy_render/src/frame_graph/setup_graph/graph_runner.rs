...
use super::{
    Edge, NodeState, SetupGraph, SetupGraphContext, SetupRunError, SlotLabel, SlotType, SlotValue,
};

pub(crate) struct SetupGraphRunner;

#[derive(Error, Debug)]
pub enum SetupGraphRunnerError {
    #[error(transparent)]
    NodeRunError(#[from] SetupRunError),
...
impl SetupGraphRunner {
    ...
    fn run_graph<'w>(
        ...
        let span = if let Some(label) = &sub_graph {
            info_span!("setup_graph", name = format!("{label:?}"))
        } else {
            info_span!("setup_graph", name = "main_graph")
        };
        #[cfg(feature = "trace")]
            ...
            for (i, input_slot) in input_node.input_slots.iter().enumerate() {
                if let Some(input_value) = inputs.get(i) {
                    if input_slot.slot_type != input_value.slot_type() {
                        return Err(SetupGraphRunnerError::MismatchedInputSlotType {
                            slot_index: i,
        ...
        'handle_node: while let Some(node_state) = node_queue.pop_back() {
                ...
                smallvec![None; node_state.output_slots.len()];
            {
                ...
                {
                    #[cfg(feature = "trace")]
                    let _span = info_span!("setup", name = node_state.type_name).entered();
